...
Three others are in the same family: wrapping an entire `==`; leaning on auto-capture where it picks a literal, as in `test[4 in mycollection]` (auto-capture takes the LHS, so the report says nothing — write `test[4 in the[mycollection]]`); and under-capturing a chained comparison. All four hide a value you would want on failure, which is what makes them bugs rather than style. Note the first and second are the same error from opposite sides — over-marking and under-marking — with the same fix. The canonical account is **`the[]`'s own docstring** (`help(the)`, or `unpythonic/syntax/testingtools.py`) — read it and pattern-match your draft against it before committing. It is canonical rather than `unpythonic/CLAUDE.md` because it is the copy library users can see, and because keeping the same guidance in two files is what let them drift apart once already.
