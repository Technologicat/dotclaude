...
- **`the` prefix names the instance, when the bare noun names the category.** The underlying move is repurposing natural English syntax as program syntax: the definite article does in code what it does in speech. In code that manipulates syntactic things generically — AST nodes, callables, bodies, expressions — `body` or `expr` reads as *some* body, while `thebody` / `theexpr` is *the one we are handling right here*. It also rescues names that are reserved words: `thelambda` exists because `lambda` cannot. Hence `thelambda`, `thecallable`, `thebody`, `thecall`, `theexpr`, `thelet` throughout `mcpyrate` and `unpythonic.syntax`. Use it where that instance-vs-category tension is real (macro and AST work, mostly); don't sprinkle it on ordinary variables, where it's just noise.

  `unpythonic`'s `the[]` capture macro shares the *linguistic* move but not the rationale: it marks which value you want to see in the failure output ("show me **the** `x`"), and there's usually only one `x` in scope, so it isn't disambiguating instance from category. Its name is also a nod to Common Lisp's `THE` — which is a *type-declaration* form, so it's a pun rather than a port. Same trick, different jobs; don't reason from one to the other.
  - **Counter-rule in test code:** never `the`-prefix a name in tests that use `unpythonic`'s `the[]` capture macro — `test[the[theresult] == 42]` isn't English. Call it `result`.
