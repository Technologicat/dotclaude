...
- **No per-file lint suppression.** Don't use `per-file-ignores` in ruff config (except `__init__.py` F401 for re-exports, which is universal). Suppress at each use site with `# noqa: CODE -- reason`. Per-file suppression is spooky action at a distance — neither a single global policy nor visible at the code site.
  ...
  - **Which is a rule about the marker's length, and it generalizes.** A marker that carries its own `-- reason` clause goes **last**, where it cannot push the code's own comment off the end of the line; a bare one-word marker — `# tag` for a DPG string tag, `# plural` for a hand-written plural — buries nothing from either end, so its position is free, and the call is **first**. Note it does not qualify the prose it precedes: the comment beside a `# tag` is usually about something else entirely. Both are about that line — that is what an end-of-line comment is — and they differ in role, the prose explaining the line to a reader and the marker flagging it for a search. So the two conventions running opposite ways is one rule, not two tastes.
  - **Neither position is a tool's requirement.** Verified 2026-09-18 against ruff 0.16.7 and flake8: both honour a `# noqa` anywhere in the trailing comment, bare or with codes, with an unsuppressed control confirming the check could fail. So don't reach for a linter to explain the ordering, and don't assume a marker must be last to work.
...
- **When a comment or docstring disagrees with the code, the default is that *both* are wrong.** Stop there. Do not resolve it in passing, and above all do not resolve it in whichever direction is cheaper to type — which is nearly always the prose, since editing a sentence feels like tidying while editing code feels like a change. Work out which of the two, if either, describes the behaviour that is actually wanted; if that is not obvious in a minute or so, raise it rather than deciding alone. Either half can be the broken one, and they fail differently: a stale sentence merely misleads the next reader, while code that was quietly bent to match a stale sentence breaks something that worked.
  - The old form is *"If the code and the comments disagree, then both are probably wrong"* — Norm Schryer of Bell Labs, collected in Jon Bentley's [*More Programming Pearls*](https://en.wikipedia.org/wiki/Programming_Pearls) (Addison-Wesley, 1988), column 6, "Bumper-Sticker Computer Science". Often misremembered as Kernighan & Plauger's; *The Elements of Programming Style* has the milder "Make sure comments and code agree", which is a different claim — it says to fix the disagreement, not to distrust both sides of it.
  - **The cheapest evidence is the rest of the surrounding prose.** A docstring is rarely wrong all over. When one sentence contradicts the code and its neighbours describe the code exactly, the odd sentence is stale — someone changed the behaviour and updated the paragraph they were looking at. When the *whole* comment describes something the code does not do, suspect the code.
  - The failure this prevents is a bug laundered into documented behaviour. A comment edited to match faulty code makes the fault look intended, and the next reader has no way to tell — the discrepancy that would have flagged it is the thing that just got deleted.
  - (Live case 2026-09-01, Raven: `_cluster_highdim_semantic_vectors` promised a returned sample of `min(len(all_vectors), max_n)` rows while returning up to twenty points *per detected cluster*. The rest of the docstring described the per-cluster behaviour correctly, which is what identified the one sentence as the stale half. Juha's call: "if docstring and code disagree, by default both are wrong.")
...
- **When the count is known, use that knowledge: agree the noun with it, never `item(s)`.** If a number is in the string, the noun beside it has a correct form available for free — `f"{n} node"` / `f"{n} nodes"`, not `f"{n} node(s)"`. This holds in **log messages** as much as in the GUI: a log line is read by a person, and `1 file(s)` is the tell of a line nobody re-read. The idiom is a one-liner (`plural_s = "s" if n != 1 else ""`); a project doing it often should factor out a helper taking the count and the singular.
  - **Agreement reaches past the noun, and a suffix helper cannot follow it there.** English also inflects the verb — `"1 record has a `doi` field"` against `"2 records have"` — and sometimes a pronoun, as in `"documents 1 key … so they are missing"`. The verb's `-s` is the *inverse* of the noun's, and the two commonest verbs are irregular (`has`/`have`, `is`/`are`), so a mirror helper only half works. **Where the count reaches a verb or a pronoun, reshape the sentence so nothing agrees with it**: a participle (`"N clusters holding more than one DOI"`), or the count moved behind a label (`"Records needing a look by hand: N"`). Leave a one-line comment saying why the phrasing is odd, or the next reader will "fix" it back. Five of these turned up in one sweep, and three were only noticed because the noun beside them was being changed.
    - **That gate is the whole of it: reshaping is for a verb or a pronoun, and for nothing else.** A count sitting beside a bare noun is the ordinary case and wants the ordinary answer — agree the noun and move on.
      - **An irregular or `-es` plural is not a reason to reshape.** Write both spellings at the use site: `f"+{n} more {'match' if n == 1 else 'matches'}"`. `plural_s` is the regular `-s` and says so; `-es` after a sibilant and `-ies` after a consonant-plus-`y` are just as regular, and `-o`, `-f`/`-fe` and the true irregulars (`index`/`indices`) are not decidable from the spelling at all. One special-cased plural costs a line. Build a helper when the second or third one turns up, and have it take and return the *noun* rather than a suffix, since `-ies` replaces a letter rather than adding one.
      - **Tag a hand-written plural `# plural`**, the way a DPG string tag is tagged `# tag`, so the day a helper is worth building the sites are greppable. **The tag goes first when the line also wants prose** — `# plural  # why this wording` — being one word, which is the `# noqa` bullet's length rule; see there for why that bullet's own marker goes the other way.
    - **The failure this gate prevents is a phrasing rule reaching into a user-facing wording decision.** Reshaping to avoid a plural looks like tidiness while it is actually choosing what a reader sees, and it does not announce itself as a design choice — so it gets applied inside a UX discussion and quietly wins, having been weighed against nothing. The words are chosen for the reader; the implementation follows, never the reverse. (Live case 2026-09-18, Raven's chat graph: a count label for a matching box was steered twice toward forms with no noun in them — the merit claimed for one candidate was literally that it had nothing to agree with — before Juha pointed out that the horizontal space was the constraint that should be deciding it, and that a special-cased plural is cheaper than worse UX. The library was already right: `raven.common.text.plural_s`'s docstring says an irregular "wants both spellings written out where it is used", and this rule had overshot it.)
  - **A float printed as a float is exempt**, singular and plural having the natural numbers as their domain: `"0.99 seconds, 1.00 second, 1.01 seconds"` is silly. Where the code prints `int(x)`, though, the number the reader sees *is* a natural number and agrees normally.
  - **A caption promising *all* of something drops the count rather than agreeing with it.** `"Copy all attachments to …"` says what the button does; `"Copy all 1 attachment"` answers a question nobody asked, the promise being *all of them, however many*.
  - **Never touch quoted or sample data.** A fixture's `"© The Author(s), under exclusive licence to …"` is the publisher's wording under test, not ours.
  - **Where the count genuinely is not known, `(s)` is telegraphic style rather than laziness** — a label like `"Attach file(s)"`, written before the user has chosen anything. No knowledge is being ignored, because there is none yet. That is the same space concession as dropping articles on an instrument panel, and it is legitimate for the same reason. **Where there is room, say it in words** — `"Attach one or more files"` — since that states outright the thing `(s)` only gestures at, namely that one is allowed. Falling back to `(s)`, or to a bare plural, is then a decision about width rather than about correctness.
  - Languages other than English have more than two forms, and this idiom does not survive translation. Not a problem while the fleet ships English — but it is the reason to keep pluralization in one helper rather than smeared across every f-string.
...
- **The sibling bullet, for asymmetry you did not mean to create.** The rule above fires when *weighing whether to add* something, which is a moment of deliberation. The commoner failure has no such moment: a change needs a special case, the special case presents itself as the solution, and nothing ever asks whether it should exist. So the trigger is the special case itself, which is always visible.
  - **When a change needs a special case, finish this sentence before writing it: *the general path doesn't work here because ___*.** If the blank names a capability one sibling has and another lacks, that is not a reason — it is the work. Give the sibling the capability, and the special case goes away. In both live cases the remedy was the same shape: a one-call entry point in the layer underneath, never a restructured caller.
    - **The sentence is consumed by asking it.** Where the asymmetry goes away there is nothing left to explain; where it survives, what the code wants is a sentence about *what makes the two genuinely different* — checkable against the code, so it defends itself — and never an account of what was tried. That history belongs in the commit message.
  - **The diff-visible variant: an edit touching both branches of a dispatch is editing the symmetry too.** If they come out different shapes, that is the finding, and no comparison against distant code is needed to see it.
  - **Symmetry decays under extension, so a one-time pass does not hold it.** `raven/client/mayberemote.py`'s `TTS.synthesize` was made symmetric on 2026-04-18 (19 lines against one call, down to one and one), re-broken three days later by adding a `format` parameter, fixed again, and then twice more for consistency with its nine sibling services. Seven rounds, each found by a person reading a diff — which is the cost this is meant to reduce, and says nothing about how many were never found, the caught ones being the only ones anybody can count.
  - **Two failures, and the second is the one that surprises.** Maintainability first: the odd-one-out costs every reader a double-take, and every later change to that area has to be made twice, in two shapes, by someone who has to work out afresh why there are two. Then correctness — untested paths accumulate in special cases, because a branch that exists for one caller is exercised the way that one caller happens to use it. The same commit that made that dispatch symmetric found a latent crash in the branch it deleted: a nil check that logged "Cancelled" and did not `return`, down a path nothing had ever taken.
  - **And name them when reporting.** Listing where two things of the same kind came out handled differently — two or three lines, in the session rather than the commit — turns the reviewer's job from searching into reading, which is the only version that scales when review attention is the binding constraint. Same shape, and the same reason, as saying which *whys* were asserted rather than sourced.
...
- **A tracked file is published, and the conversation is not.** Most of the fleet is public and the rest could be tomorrow, so committing something is the act that makes it public, whatever it was fine to say while working it out. Machines go in by role rather than hostname, private corpora by shape rather than name or count, and nothing about a person that `AUTHORS.md` and the git history do not already carry. Same boundary a human developer keeps by instinct, and it is worth keeping because the alternative is the maintainer watching every word in the session against the chance that any of it lands in a repo — which costs far more than the few words it saves. Two rules in this fleet are cases of it: *name machines by role* in the `~/.claude` section below, and Raven's *name the shape, not the dataset*. Neither is scoped to its own repo in principle; both apply everywhere.
...
- **Don't stay on an old version without a reason, and write the reason at the pin.** A dependency pin or floor that lags the current release wants one of three justifications: **(a)** the new version doesn't work, **(b)** it isn't compatible with the rest of the dependency tree, or **(c)** moving to it needs porting work that isn't affordable right now. Absent one of those, take the new version — in the CI pins, the lockfile and the floors alike, a floor being a claim about what we *test* rather than about the oldest release we would support.
  - **(c) is the one that actually recurs, and the one most often left unwritten**, because at the moment of deferring it the reason feels too obvious to need saying. It stops being obvious within the week.
  - The failure this prevents is a pin that lags for no recorded reason, which reads exactly like neglect: the next person can't tell a deliberate deferral from an oversight, so they either spend the investigation over again or leave it standing forever.
  - **Dependabot's proposal is not automatically the newest release.** It opens a PR for what it computed at scan time, and a release landing between that scan and the merge leaves the pin a version behind with nothing saying so. Check the current release before merging, not just the diff. (Live case 2026-09-20: a fleet sweep took `codecov-action` 7.1.0 in two repos while 7.1.1 had been out three days, and `filelock` 4.0.0 in Raven while 4.0.1 had been out a day.)
